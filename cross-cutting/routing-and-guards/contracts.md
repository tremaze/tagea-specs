...
| `requireFileStorageAccess` (with `requireFeature('fileStorage')`) | `auth-session/authz-guards.ts` | `isFeatureEnabled('fileStorage')` | `/dateien`: `canInAnyTeamspace('files.view')` (incl. bridge `tenant.teamspaces.all.files.view`); `/einrichtung/:id/dateien`: `canInInstitution(id, 'institution.files.view')`. `file_storage.access` is no longer checked (#4464) | backend-computed landing (`SessionRouter`) |
