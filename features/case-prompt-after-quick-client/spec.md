...
- [ ] **Given** the user saves the Quick Create Case form, **Then** the new case is persisted with `status = 'active'` — same as the regular case dialog. The DB-level default of `draft` only applies when callers omit the field; the slim panel sets it explicitly to keep parity with the normal creation flow (a freshly-attached case is by definition active, not a draft).
