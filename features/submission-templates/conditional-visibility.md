...
## §10 Umsetzungsstand (Ist, 2026-06-16)

PR-1 bis PR-4 sind umgesetzt und gemerged (`eddfb391f feat(submissions): conditional visibility for Meldungen`): Feld- und Sektions-Authoring (`VisibilityConditionEditor`, `buildUiConfig`-Merge-Fix, `@Allow()` auf `VisibilityConditionDto.value`), per-Row-Renderer (D3), Server-Strip `strip_invisible_submission_values` auf allen sechs Schreibpfaden (Migration `20260612120000`). PR-5 (Altdaten-Backfill) bleibt offen/optional.

**Nachgezogene Korrekturen (Follow-up, gegen den §5-Plan):**
- **GAP-FE-2 (Read-only):** Repeating-Rows waren schon in Phase-E visibility-sauber (Skip leerer Werte). Die **flache** Read-only-Schleife (`submission-detail-page.component.ts`) blieb im `eddfb391f`-Commit unangetastet und zeigte gestrippte Conditional-Felder als „–"-Zeilen. Behoben via `isHiddenConditional(field)` (skippt ein Feld mit `visibility_condition`, dessen Wert `'-'` ist — spiegelt den Repeating-Row-Pfad). Rein kosmetisch; D1 hatte den Wert bereits entfernt.
- **GAP-FE-5 (Single-Flat-Group):** `transformToFieldGroups` (`teamspace-submissions-page.component.ts`) bleibt bewusst unverändert. Nur erreichbar bei ≤1 Gruppe ohne Repeating; Same-Group-Feld-Conditions funktionieren dort weiterhin (sie reisen im Feld-Objekt mit und werden über die flache Controls-Map aufgelöst). **Akzeptiertes Rest-Risiko:** eine *Group-level*-Condition auf einer 1-Gruppen-Vorlage greift in dieser Ansicht nicht (nahezu sinnlose Konfiguration).
- **F1 (neuer Befund, Renderer):** Wird eine Sektion mit Group-Condition wieder sichtbar, schaltete der geteilte Renderer **alle** ihre Felder unbedingt sichtbar und übersprang die *eigene* Feld-Condition (deren Subscription erst beim nächsten Trigger-Wechsel re-evaluiert). Submissions erlauben jetzt erstmals Group- **und** Feld-Conditions zusammen → erreichbar. Behoben über `computeFieldVisible`/`applyFieldVisibility` in `setupVisibilityConditions` (Group∧Feld, konsistent mit dem Feld-Level-Zweig) + Nested-Unit-Test in `tagea-custom-fields.visibility.spec.ts`. **Daten waren nie betroffen** (D1 wertet Group∧Feld unabhängig aus und strippt korrekt) — es war ein UX/Verwirrungs-Bug. Der Fix trifft den geteilten Renderer und verbessert clients/cases mit.

> **Spec-Status:** PR-1..4 umgesetzt + gemerged (`eddfb391f`), Follow-up-Korrekturen §10 angewandt. Quelle der ursprünglichen Befunde: Readiness-Audit 2026-06-12 (Workflow `submissions-conditional-visibility-audit`, 33 Agenten, file:line-verifiziert). Memory-Pointer: `project_submissions_conditional_visibility_audit`.
