...
- [ ] **Given** an appointment is a recurring series (`recurrence_rule IS NOT NULL`, `anchor_appointment_id IS NULL` — the anchor) and the requesting employee is **not** a staff participant on the anchor, **When** the calendar endpoint runs, **Then** neither the anchor itself nor any of its virtual occurrences are returned. The participant filter applies to the anchor record before virtual occurrences are generated; the anchor's `institution_id` and `teamspace_id` are not read-time gates here either.
- [ ] **Given** the requesting employee is a staff participant on an anchor, **When** virtual occurrences are generated for the requested time range, **Then** every generated occurrence inherits the anchor's visibility — i.e. all occurrences within the range are returned (subject to `excluded_dates` and `recurrence_end_date`).
- [ ] **Given** the caller hits `GET /appointments/:id/occurrences/:date` (virtual-occurrence detail), **When** the service resolves the anchor, **Then** the same visibility rule as for `findOne` applies — the anchor must be visible to the caller (institution match, or staff-participant on a non-institution anchor). Otherwise the endpoint responds with 404.
...
- [ ] **Given** a staff invitee RSVPs on `/teamspace/kalender/:id` (Accept or Decline), **When** the request fires, **Then** it hits the self-RSVP route that matches the appointment: `PATCH /teamspaces/:teamspaceId/appointment-participants/:id` when `teamspace_id` is set, otherwise the tenant-level `PATCH /appointment-participants/:id` (the normal case — teamspace-calendar appointments persist with `teamspace_id IS NULL`). Institution appointments (`institution_id` set, `teamspace_id` null) are answered in the institution context (`PATCH /institutions/:institutionId/appointment-participants/:id`, needs institution rights); the mobile teamspace detail shows only the own status for them (Angular redirects to the institution instead). The RSVP must work for employees without any institution assignment. See [appointment-detail spec — RSVP section](../appointment-detail/spec.md) for the full semantics.
